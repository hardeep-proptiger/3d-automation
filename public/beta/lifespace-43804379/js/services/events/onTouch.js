...
function onTouch(event) {
    ...
    } else if( "pinchin" === event.type ){
        g_cRenderer.modelWalk(g_DOWNKEY)
    } else if( "pinchout" === event.type ){
        g_cRenderer.modelWalk(g_UPKEY)
    } else "singletap" === event.type && ( event.preventDefault(), window.checking=event, g_cRenderer.mouseClick(event.center.x, event.center.y))
};