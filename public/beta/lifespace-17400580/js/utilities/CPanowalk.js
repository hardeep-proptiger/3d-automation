...
CPanowalk.prototype = {
    ...
    jumpToPosition: function(a) {
        function b() {
            c.updateMarkers(a);
            c.m_cNextSphere.getSphereModel().visible = !0;
            var b = c.m_cNextSphere.m_center;
            c.m_cCamera.position.set(b.x, b.y, b.z);
            c.updateRotation(0, 0);
            c.m_cCurrSphere.getSphereModel().visible = !1;
            c.m_cPanoverse.moveToNode(a)
        }
        if (!this.m_fIsInTransit && (this.m_cCurrSphere = this.m_cPanoverse.getCurrentSphere(), this.m_cNextSphere = this.m_cPanoverse.getSphereByIndex(a), this.m_cNextSphere.loadSphere(), this.m_cPanoverse.isValidNode(a))) {
            var c = this,
                d = this.m_cNextSphere.getTexLoadStatus();
            waitForCondition(d, !0, b)
        }
    },
